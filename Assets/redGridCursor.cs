...
{
    // Grid variables (initialized using the newGridManager's gridParent)
    ...
    {
        // Wait until newGridManager's gridParent becomes available.
        Transform gp = null;
        while (gp == null || gp.childCount == 0)
        {
            gp = FindFirstObjectByType<newGridManager>()?.gridParent;
            yield return null;
        }
    ...
    void Update()
    {
        ...
        if (!hasJoined)
        {
            if (newGridManager.Instance.RedInGrid)
            {
                hasJoined = true;
            }
            return;
        ...
        if (newGridManager.Instance != null && newGridManager.Instance.RedInGrid && !autoProcessStarted)
        {
            autoProcessStarted = true;
            StartCoroutine(AutoProcess());
        }
        Debug.Log($"The current state of RedINGRID is [{newGridManager.Instance.RedInGrid}].");
    }
    ...
    {
        List<newBuildingSlot.BuildingRecord> records = newBuildingSlot.GetAllBuildingsOnBoard();
    ...
    IEnumerator AutoProcess()
    {
        ...
        newGridManager gm = newGridManager.Instance;
        ...
        newBuildingSlot targetSlot = targetCell.GetComponent<newBuildingSlot>();
        if (targetSlot == null)
        {
    ...
    Vector2Int CalculateDestination()
    {
        ...
        List<newBuildingSlot.BuildingRecord> allRecords = newBuildingSlot.GetAllBuildingsOnBoard();
        // Get flag records.
        List<newBuildingSlot.BuildingRecord> flagRecords = new List<newBuildingSlot.BuildingRecord>();
        ...
        foreach (var flag in flagRecords)
        {
            foreach (Vector2Int offset in candidateOffsets)
            {
                ...
                if (candidate.x >= 0 && candidate.x < rows && candidate.y >= 0 && candidate.y < cols)
                {
                    ...
                    {
                        newBuildingSlot slot = cells[candidate.x, candidate.y].GetComponent<newBuildingSlot>();
        ...
        for (int r = 0; r < rows; r++)
        {
            for (int c = 0; c < cols; c++)
            {
                if (cells[r, c] != null)
                {
                    newBuildingSlot slot = cells[r, c].GetComponent<newBuildingSlot>();
                    if (slot != null && !slot.HasBuilding())
                    {
    ...
    void ReactivatePlayer()
    {
        ...
        if (newGridManager.Instance != null && newGridManager.Instance.gridParent != null)
        {
            newGridCellActivation[] activations = newGridManager.Instance.gridParent.GetComponentsInChildren<newGridCellActivation>();
            foreach (newGridCellActivation act in activations)
            {
                act.redInGrid = false;
            }
        }
